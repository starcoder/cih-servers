...
class Geography(StarcoderVisualization):

    def __init__(self, *argv, **argd):
        super(Geography, self).__init__(*argv, **argd)
    ...
    def data(self):
        ...
        #for i in range(len(fd[0]["values"])):
        #    fd[0]["values"][i]["latitude"] = 1.0 #[d for d in fd[0]["values"]]
        #    fd[0]["values"][i]["longitude"] = 1.0 #[d for d in fd[0]["values"]]
        fd[0]["transform"] = [
            {
               "type": "filter",
              "expr": "datum[independent] != null" #.format(self.ifield_name)
               #"expr": "datum.{} != null && datum.{} != null".format(self.ifield_name, self.dfields[0])
            },
            {
                "type" : "formula",
                "as" : "latitude",
                "expr" : "datum[independent].latitude",
            },
            {
                "type" : "formula",
                "as" : "longitude",
                "expr" : "datum[independent].longitude",
            },
            {
                "type": "geopoint",
                "projection": "overall",
            ...
            {
               "type": "filter",
               "expr": "datum.longitude != null && datum.latitude != null" #.format(self.ifield)
            }
        ]
        return [
            {
