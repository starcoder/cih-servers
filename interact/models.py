...
def make_name(project_id, entity_type_name):
    return "cih_{}^{}".format(project_id, entity_type_name).replace("_", " ").title().replace(" ", "").replace("^", "_")
...
model_mapping = {
    ...
    "categorical" : (models.CharField, {"max_length" : 1500, "null" : True}),
    "boolean" : (models.CharField, {"max_length" : 1500, "null" : True}),
...
for project in projects:
    try:
        ...
        fields = {}    
        for field_name, field_spec in schema.get("data_fields", {}).items():
        ...
        for entity_type_name, entity_type_spec in schema["entity_types"].items():
            ...
            for field_name in entity_type_spec.get("data_fields", []):
            ...
            rclass_attr["bottleneck"] = models.JSONField(null=True)
